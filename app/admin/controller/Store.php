    ...
    public function publish()
    {
        ...
        if ($id > 0) {
            ...
            if ($this->request->isPost()) {
                ...
                //删除之前的服务
                Db::name('storeservice')->where('storeid',$id)->delete();

                //添加现在选择的服务
                foreach ($post['servicetypeid'] as $k=>$v){
                    $where1['storeid']=$id;
                    $where1['servicetypeid']=$v;
                    DB::name('storeservice')->insert($where1);
                }
                ...
                //选择的服务类目
                $servicetypeid=Db::name('storeservice')->where('storeid',$id)->select();
                $this->assign('servicetypeid', $servicetypeid);
