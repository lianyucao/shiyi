...
class Main extends Permissions
{
    public function index()
    {
        ...
        $web['totalmember'] = Db::name('user_member')->where('level_status',1)->count();
        $web['new_user_num'] = Db::name('user')->where('create_at', '>=', strtotime('yesterday'))->count();
        $web['totalcoefficient'] = Db::name('webconfig')->find()['totalcoefficient'];
        $web['review_user'] = Db::name('user_member')->where('status', 0)->count();
        $web['review_store'] = Db::name('store')->where('status', 0)->count();
        $web['review_service_order'] = Db::name('service_order')->where('status', 0)->count();
        $web['review_pay'] = Db::name('service_order')->where('status', 2)->count();
        ...
        foreach ($date as $k => $val) {
            ...
            $login_sum.= Db::name('user')->where(['level'=>1])->where($where)->count() . ',';
        }
